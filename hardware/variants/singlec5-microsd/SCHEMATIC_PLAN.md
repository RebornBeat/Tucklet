# Schematic Plan (net-by-net) — singlec5-microsd

Build this in KiCad: place the symbols from BOM.csv, then wire the nets
below. Or import `tucklet-singlec5-microsd.net` into Pcbnew to start the
board with a ratsnest directly (logical netlist; reconcile module pins
via PIN_MAP.md).

Variant: **Single ESP32-C5**, **microSD (swappable)** storage.
