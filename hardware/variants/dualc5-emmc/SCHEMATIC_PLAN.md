# Schematic Plan (net-by-net) — dualc5-emmc

Build this in KiCad: place the symbols from BOM.csv, then wire the nets
below. Or import `tucklet-dualc5-emmc.net` into Pcbnew to start the
board with a ratsnest directly (logical netlist; reconcile module pins
via PIN_MAP.md).

Variant: **Dual ESP32-C5 (link-aggregated, experimental)**, **eMMC (sealed)** storage.
